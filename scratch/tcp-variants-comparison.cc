...
static Ptr<OutputStreamWrapper> bbrStateStream;
static Ptr<OutputStreamWrapper> pacingRateStream;
...
static void
BbrStateTracer (TcpBbr::BbrMode_t old, TcpBbr::BbrMode_t newMode)
{
  NS_UNUSED (old);
  *bbrStateStream->GetStream () << Simulator::Now ().GetSeconds () << " " << newMode << std::endl;
}
static void
TraceBbrState (std::string &bbr_state_file_name)
{
  AsciiTraceHelper ascii;
...
// add tracer for pacing rate
static void
PacingRateTracer (DataRate oldValue, DataRate newValue)
{
  NS_UNUSED(oldValue);
  *pacingRateStream->GetStream () << Simulator::Now ().GetSeconds () << " " << newValue.GetBitRate () << std::endl;
}
static void
TracePacingRate (std::string &pacing_rate_file_name)
{
  AsciiTraceHelper ascii;
